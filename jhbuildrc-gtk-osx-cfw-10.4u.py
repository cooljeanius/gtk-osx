...
_ppc_args = "CFLAGS='" + _cflags + " -arch ppc' CXXFLAGS='" + _cxxflags + " -arch ppc' LDFLAGS='" + _ldflags + " -arch ppc -headerpad_max_install_names' --host=powerpc-apple-darwin "

_i386_args = "CFLAGS='" + _cflags + " -arch i386' CXXFLAGS='" + _cxxflags + " -arch i386' LDFLAGS='" + _ldflags + ",-arch i386,-headerpad_max_install_names' --host=i386-apple-darwin "

_genmarshal = "/Library/Frameworks/GLib.framework/Versions/2/Resources/dev/bin/glib-genmarshal"

append_autogenargs("glib-ppc", _ppc_args + " glib_cv_uscore=no ac_cv_func_posix_getgrgid_r=yes glib_cv_stack_grows=no ac_cv_func_posix_getpwuid_r=yes ac_cv_c_bigendian=yes GLIB_GENMARSHAL=" + _genmarshal)

append_autogenargs("glib-i386", _i386_args + " glib_cv_uscore=no ac_cv_func_posix_getgrgid_r=yes glib_cv_stack_grows=no ac_cv_func_posix_getpwuid_r=yes ac_cv_c_bigendian=no GLIB_GENMARSHAL=" + _genmarshal)
...
_pixbuf_csource = checkoutroot + "/gtk-u/ppc/gdk-pixbuf/gdk-pixbuf-csource GDK_PIXBUF_MODULE_FILE=" + checkoutroot + "/gtk-u/ppc/gdk-pixbuf/gdk-pixbuf.loaders "

append_autogenargs("gtk-ppc", _ppc_args + " --with-gdktarget=quartz --without-libjasper --disable-glibtest gio_can_sniff='yes' GTK_UPDATE_ICON_CACHE=" + _update_icon_cache + " GDK_PIXBUF_CSOURCE=" + _pixbuf_csource)
append_autogenargs("gtk-i386", _i386_args + " --with-gdktarget=quartz --without-libjasper --disable-glibtest gio_can_sniff='yes' GTK_UPDATE_ICON_CACHE=" + _update_icon_cache + " GDK_PIXBUF_CSOURCE=" + _pixbuf_csource)
